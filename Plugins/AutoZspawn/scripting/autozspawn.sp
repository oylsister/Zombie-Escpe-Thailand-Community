...
public Action ToggleAutoZspawn(int client, int args)
{
	...
	CPrintToChat(client, "{green}[ZR]{default} %T %T", "ToggleAutoZspawn", g_autozspawn[client] ? "Enabled" : "Disabled", "for you");
...
public Action Event_OnPlayerTeam(Handle event, const char[] name, bool dontBroadcast)
{
    	...
    	if(!IsPlayerAlive(client) && iTeam >= 2)
	{
		...
		CPrintToChat(client, "{green}[ZR]{default} %T %T, "JoinTeam", g_autozspawn[client] ? "Enabled" : "Disabled", "TypeAgain");
        }
    	return Plugin_Continue;
}
